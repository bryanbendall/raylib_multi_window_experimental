...
void PollInputEvents(void)
{
    ...
    while ((pollResult = ALooper_pollAll(platform.appEnabled? 0 : -1, NULL, &pollEvents, (void**)&platform.source)) >= 0)
    {
        ...
        {
            //CORE.Window[0].shouldClose = true;
...
int InitPlatform(void)
{
    ...
    //----------------------------------------------------------------------------
    CORE.Window[0].currentFbo.width = CORE.Window[0].screen.width;
    CORE.Window[0].currentFbo.height = CORE.Window[0].screen.height;
    ...
    if (CORE.Window[0].screen.width <= CORE.Window[0].screen.height)
    {
    ...
    while (!CORE.Window[0].ready)
    {
        ...
        while ((pollResult = ALooper_pollAll(0, NULL, &pollEvents, (void**)&platform.source)) >= 0)
        {
            ...
            // NOTE: Never close window, native activity is controlled by the system!
            //if (platform.app->destroyRequested != 0) CORE.Window[0].shouldClose = true;
        }
    }
...
static int InitGraphicsDevice(void)
{
    ...
    ANativeWindow_setBuffersGeometry(platform.app->window, CORE.Window[0].render.width, CORE.Window[0].render.height, displayFormat);
...
static void AndroidCommandCallback(struct android_app *app, int32_t cmd)
{
    switch (cmd)
    {
            ...
            if (app->window != NULL)
            {
                if (platform.contextRebindRequired)
                {
                    ...
                    ANativeWindow_setBuffersGeometry(app->window,
                        CORE.Window[0].render.width + CORE.Window[0].renderOffset.x,
                        CORE.Window[0].render.height + CORE.Window[0].renderOffset.y,
                        displayFormat);
                ...
                {
                    CORE.Window[0].display.width = ANativeWindow_getWidth(platform.app->window);
                    CORE.Window[0].display.height = ANativeWindow_getHeight(platform.app->window);

                    // Initialize graphics device (display device and OpenGL context)
                    InitGraphicsDevice();

                    // Initialize OpenGL context (states and resources)
                    // NOTE: CORE.Window[0].currentFbo.width and CORE.Window[0].currentFbo.height not used, just stored as globals in rlgl
                    rlglInit(CORE.Window[0].currentFbo.width, CORE.Window[0].currentFbo.height);
        ...
        case APP_CMD_GAINED_FOCUS:
        {
            ...
            CORE.Window[0].flags &= ~FLAG_WINDOW_UNFOCUSED;
            //ResumeMusicStream();
        } break;
...
static int32_t AndroidInputCallback(struct android_app *app, AInputEvent *event)
{
    ...
    for (int i = 0; (i < CORE.Input.Touch.pointCount) && (i < MAX_TOUCH_POINTS); i++)
    {
        ...
        // Normalize CORE.Input.Touch.position[i] for CORE.Window[0].screen.width and CORE.Window[0].screen.height
        float widthRatio = (float)(CORE.Window[0].screen.width + CORE.Window[0].renderOffset.x) / (float)CORE.Window[0].display.width;
        float heightRatio = (float)(CORE.Window[0].screen.height + CORE.Window[0].renderOffset.y) / (float)CORE.Window[0].display.height;
        CORE.Input.Touch.position[i].x = CORE.Input.Touch.position[i].x * widthRatio - (float)CORE.Window[0].renderOffset.x / 2;
        CORE.Input.Touch.position[i].y = CORE.Input.Touch.position[i].y * heightRatio - (float)CORE.Window[0].renderOffset.y / 2;
    }
