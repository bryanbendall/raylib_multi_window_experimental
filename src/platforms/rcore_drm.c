    ...
    // NOTE: Replaces the EventThread handling that is now commented.
    {
        ...
        while (read(fd, &event, sizeof(event)) == (int)sizeof(event))
        {
            ...
            if (event.type == EV_ABS)
            {
                ...
                if (event.code == ABS_X)
                {
                    CORE.Input.Mouse.currentPosition.x = (event.value - platform.absRange.x)*CORE.Window[0].screen.width/platform.absRange.width;    // Scale according to absRange
                    CORE.Input.Touch.position[0].x = (event.value - platform.absRange.x)*CORE.Window[0].screen.width/platform.absRange.width;        // Scale according to absRange
                ...
                if (event.code == ABS_Y)
                {
                    CORE.Input.Mouse.currentPosition.y = (event.value - platform.absRange.y)*CORE.Window[0].screen.height/platform.absRange.height;  // Scale according to absRange
                    CORE.Input.Touch.position[0].y = (event.value - platform.absRange.y)*CORE.Window[0].screen.height/platform.absRange.height;      // Scale according to absRange
                ...
                {
                    if (platform.touchSlot < MAX_TOUCH_POINTS) CORE.Input.Touch.position[platform.touchSlot].x = (event.value - platform.absRange.x)*CORE.Window[0].screen.width/platform.absRange.width;    // Scale according to absRange
                }

                if (event.code == ABS_MT_POSITION_Y)
                {
                    if (platform.touchSlot < MAX_TOUCH_POINTS) CORE.Input.Touch.position[platform.touchSlot].y = (event.value - platform.absRange.y)*CORE.Window[0].screen.height/platform.absRange.height;  // Scale according to absRange
                }
            ...
            if (!CORE.Input.Mouse.cursorHidden)
            {
                ...
                if (CORE.Input.Mouse.currentPosition.x > CORE.Window[0].screen.width/CORE.Input.Mouse.scale.x) CORE.Input.Mouse.currentPosition.x = CORE.Window[0].screen.width/CORE.Input.Mouse.scale.x;

                if (CORE.Input.Mouse.currentPosition.y < 0) CORE.Input.Mouse.currentPosition.y = 0;
                if (CORE.Input.Mouse.currentPosition.y > CORE.Window[0].screen.height/CORE.Input.Mouse.scale.y) CORE.Input.Mouse.currentPosition.y = CORE.Window[0].screen.height/CORE.Input.Mouse.scale.y;
            }
...
int InitPlatform(void)
{
    ...
    if ((CORE.Window[0].screen.width <= 0) || (CORE.Window[0].screen.height <= 0))
    {
        ...
        CORE.Window[0].screen.width = CORE.Window[0].display.width;
        CORE.Window[0].screen.height = CORE.Window[0].display.height;
    }

    const bool allowInterlaced = CORE.Window[0].flags & FLAG_INTERLACED_HINT;
    const int fps = (CORE.Time.target > 0)? (1.0/CORE.Time.target) : 60;

    // Try to find an exact matching mode
    platform.modeIndex = FindExactConnectorMode(platform.connector, CORE.Window[0].screen.width, CORE.Window[0].screen.height, fps, allowInterlaced);

    // If nothing found, try to find a nearly matching mode
    if (platform.modeIndex < 0) platform.modeIndex = FindNearestConnectorMode(platform.connector, CORE.Window[0].screen.width, CORE.Window[0].screen.height, fps, allowInterlaced);

    // If nothing found, try to find an exactly matching mode including interlaced
    if (platform.modeIndex < 0) platform.modeIndex = FindExactConnectorMode(platform.connector, CORE.Window[0].screen.width, CORE.Window[0].screen.height, fps, true);

    // If nothing found, try to find a nearly matching mode including interlaced
    if (platform.modeIndex < 0) platform.modeIndex = FindNearestConnectorMode(platform.connector, CORE.Window[0].screen.width, CORE.Window[0].screen.height, fps, true);
    ...
    CORE.Window[0].display.width = platform.connector->modes[platform.modeIndex].hdisplay;
    CORE.Window[0].display.height = platform.connector->modes[platform.modeIndex].vdisplay;
    ...
    //  -> CORE.Window[0].screen.width/CORE.Window[0].screen.height
    //  -> CORE.Window[0].render.width/CORE.Window[0].render.height
    //  -> CORE.Window[0].screenScale
    ...
    else SetWindowPosition(GetMonitorWidth(GetCurrentMonitor()) / 2 - CORE.Window[0].screen.width / 2, GetMonitorHeight(GetCurrentMonitor()) / 2 - CORE.Window[0].screen.height / 2);

    // Set some default window flags
    CORE.Window[0].flags &= ~FLAG_WINDOW_HIDDEN;       // false
    CORE.Window[0].flags &= ~FLAG_WINDOW_MINIMIZED;    // false
    CORE.Window[0].flags |= FLAG_WINDOW_MAXIMIZED;     // true
    CORE.Window[0].flags &= ~FLAG_WINDOW_UNFOCUSED;    // false
...
void ClosePlatform(void)
{
    ...
    // because they are controlled by CORE.Window[0].shouldClose variable

    CORE.Window[0].shouldClose = true;   // Added to force threads to exit when the close window is called
...
static void ProcessKeyboard(void)
{
    ...
    if (CORE.Input.Keyboard.currentKeyState[CORE.Input.Keyboard.exitKey] == 1) CORE.Window[0].shouldClose = true;
...
/*
    ...
    while (!CORE.Window[0].shouldClose)
    {
        ...
        while (read(worker->fd, &event, sizeof(event)) == (int)sizeof(event))
        {
            ...
            if (event.type == EV_ABS)
            {
                ...
                if (event.code == ABS_X)
                {
                    CORE.Input.Mouse.currentPosition.x = (event.value - worker->absRange.x)*CORE.Window[0].screen.width/worker->absRange.width;    // Scale according to absRange
                    CORE.Input.Touch.position[0].x = (event.value - worker->absRange.x)*CORE.Window[0].screen.width/worker->absRange.width;        // Scale according to absRange
                ...
                if (event.code == ABS_Y)
                {
                    CORE.Input.Mouse.currentPosition.y = (event.value - worker->absRange.y)*CORE.Window[0].screen.height/worker->absRange.height;  // Scale according to absRange
                    CORE.Input.Touch.position[0].y = (event.value - worker->absRange.y)*CORE.Window[0].screen.height/worker->absRange.height;      // Scale according to absRange
                ...
                if (event.code == ABS_MT_POSITION_X)
                {
                    if (worker->touchSlot < MAX_TOUCH_POINTS) CORE.Input.Touch.position[worker->touchSlot].x = (event.value - worker->absRange.x)*CORE.Window[0].screen.width/worker->absRange.width;    // Scale according to absRange
                }

                if (event.code == ABS_MT_POSITION_Y)
                {
                    if (worker->touchSlot < MAX_TOUCH_POINTS) CORE.Input.Touch.position[worker->touchSlot].y = (event.value - worker->absRange.y)*CORE.Window[0].screen.height/worker->absRange.height;  // Scale according to absRange
                }
